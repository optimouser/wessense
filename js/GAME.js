  ...
  amplify.subscribe('PLAYER_MOVED', function() {
    ...
    if ( obj !== undefined ) {
      if ( obj.t === 'mine' && obj.mudcrawler_released === false && GAME.data['role'].startsWith('role_3')) {
        ...
        var hostage = GAME.Monsters.Create( GAME.data['hostage'], pos.x, pos.y );
        hostage.quest_hostage = true;
        GAME.player.mStats.hostage_released = true;
        GAME.Notifications.Post('Quest Progress: Hostage Released!', 'good');
      }
    }
...
GAME.ProcessMonsterAction = function( monster, next_monster_move ) {
  ...
  if ( monster.mRegeneration !== undefined && monster.mHP[0] < monster.mHP[1] ) {
    // normal regeneration
    var hp = Math.min(monster.mHP[1] - monster.mHP[0], monster.mRegeneration);
    monster.mHP[0] += hp;
    ...
    var hp = Math.min(monster.mHP[1] - monster.mHP[0], 8);
    monster.mHP[0] += hp
    if ( monster.IsOffScreen() === false ) {
...
GAME.GameOver = function( good ) {
  // TODO clear amplify subscriptions?
  GAME.mGameOver = true;
    GAME.StopLoop();
    GAME.mDisableControls = true;
